    ...
    <ion-content :fullscreen="true">
      ...
      <div class="body">
        <div class="window form">
          <div class="title-bar">
            <div class="title-bar-text">Region.exe</div>
          ...
          <div class="window-body input">
            <div class="field-row-stacked" style="width: 69.44444444444444vw">
              <label for="text22">Rechercher une région :</label>
              <select id="text22" v-model="q">
                <option v-for="region in regions" :key="region" :value="region.code" >{{ region.code }} - {{ region.nom }}</option>
              </select>
        ...
        <div class="window departments" style="width: 83.3vw" v-show="results.length > 0">
          ...
          <div class="window department" v-for="result in results" :key="result">
            <div class="title-bar">
              <div class="title-bar-text">{{ result.code }} - {{ result.nom }}</div>
            </div>
            <div class="window-body">
              <div class="errorMessage">
                <table>
                  <thead>
                    <tr>
                      <th colspan="2">{{ result.code }} - {{ result.nom }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
        ...
        <div class="window help" style="width: 83.3vw" v-show="showHelp">
          ...
          <div class="window-body">
            <p>  
              ...
              Sélectionnez un code de région (ex:11) pour afficher tout les départements de la région.<br>
              Sélectionner une région qui n'existe pas ou même un faux code de région entrainera également l'apparition d'une fenêtre d'erreur.<br><br>
  ...
  methods : {
    search(){
      if (this.q.length === 0) {
        ...
        this.errorMessage = "Afin d'effectuer une recherche, vous devez impérativement choisir l'une des régions proposé !"
        return
      }

      if (isNaN(parseInt(this.q))){
        this.showError = !this.showError
        this.errorMessage = "Code de région invalide !"
        return
      }

      fetch(`https://geo.api.gouv.fr/regions/${this.q}/departements`)
        .then(response => {
          if (response.status == 404) {
            this.showError = !this.showError
            this.errorMessage = `Pas de résultat pour le code de région suivant : ${this.q} !`
            return
          }
          response.json()
          .then(data=>{
